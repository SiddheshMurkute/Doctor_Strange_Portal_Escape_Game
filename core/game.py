...
class Game:
    ...
    def __init__(self):
        ...
        self.portal_result_ui = PortalResult(
            self.font_large,
            self.font_med,
            self.font_small
        )

        self.final_escape_ui = FinalEscapeScreen(
            self.font_large,
            self.font_med
        )

        self.final_result_ui = FinalResultScreen(
            self.font_large,
            self.font_med,
            self.font_small
        )

        self.failure_ui = FailureScreen(
            self.font_large,
            self.font_med,
            self.font_small
        )

        self.stage_intro_ui = None
    ...
    def _start_stage(self, stage: int = None):
        ...
        self.stage_intro_ui = StageIntro(
            self.font_large,
            self.font_med,
            self.font_small,
            s,
    ...
    def _restart_game(self):
        ...
        self.lm.difficulty = (
            self.diff_menu.options[
                self.diff_menu.selected
            ]
        )
    ...
    def _handle_state_event(
        ...
        if s == GameState.MAIN_MENU:
            ...
            result = self.main_menu.handle_event(
                event,
                mouse
            )
        ...
        # ---------------------------------------------------------
            ...
            result = self.diff_menu.handle_event(
                event,
                mouse
            )
        ...
        # ---------------------------------------------------------
            ...
            result = self.how_to_play.handle_event(
                event
            )
        ...
        # ---------------------------------------------------------
            ...
            result = self.stage_intro_ui.handle_event(
                event
            )
        ...
        # ---------------------------------------------------------
            ...
            result = self.final_result_ui.handle_event(
                event,
                mouse
            )
        ...
        # ---------------------------------------------------------
            ...
            result = self.failure_ui.handle_event(
                event,
                mouse
            )
    ...
    def _update_playing(
        ...
        if result == "correct":
            ...
            time_bonus = int(
                self.time_left *
                SCORING["time_bonus_per_sec"]
            )

            self.lm.add_score(
                SCORING["correct_portal_base"]
                + time_bonus
            )
            ...
            self.portal_result_ui.show(
                "correct",
                0,
                duration=2.2
            )

            self._portal_result_result = (
                "correct"
            )

            self.state = (
                GameState.PORTAL_RESULT
            )

        elif result == "wrong":

            self.lm.add_score(
                SCORING["wrong_portal_penalty"]
            )
    ...
    def _advance_or_final(self):
        ...
        if self.lm.is_final_stage():
            ...
            self.state = (
                GameState.FINAL_ESCAPE
            )

        else:

            self.lm.add_score(
                SCORING["stage_complete_bonus"]
            )
    ...
    def _draw(self):
        ...
        if s == GameState.MAIN_MENU:
            ...
            s == GameState.STAGE_INTRO
            and self.stage_intro_ui
        ):

            self.stage_intro_ui.draw(
                self.screen
            )
            ...
            self.final_escape_ui.draw(
                self.screen
            )

        elif s == GameState.FINAL_RESULT:

            self.final_result_ui.draw(
                self.screen,
                self.lm.total_score,
    ...
    def _draw_gameplay(self):
        ...
        self.level.draw_interact_prompts(
            self.screen,
            self.player.rect,
            self.font_small
        )
